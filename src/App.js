...
function MakeForm() {
  ...
  const { directions, coords, xid } = useDirections(result, currentLocation, rad);
  ...
  return (
    ...
    >
      ...
      <Box sx={{ m: 2 }}>
        <Button variant="contained" size="medium" onClick={() => { setRad(radius.current.value); setResult(!result); }}>
          Generate
        </Button>
      </Box>
      <Box sx={{
